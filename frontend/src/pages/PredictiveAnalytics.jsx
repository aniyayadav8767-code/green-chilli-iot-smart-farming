...
import { LineChart, Sparkles, BrainCircuit, Droplets, ShieldAlert, CheckCircle2, Loader2 } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3 bg-white border border-slate-200/80 rounded-2xl">
        ...
        <p className="text-xs text-slate-500 font-extrabold">Synthesized projections and soil profiles running...</p>
      </div>
    );
  }

  // Fallback if data is empty or fail response
  const p = data || {
    healthIndex: 92,
    diseaseRiskScore: 15,
    waterRequirement: '0 Liters',
    aiRecommendation: 'Soil moisture profile is healthy and stable. Keep current schedule.',
    soilDepletionForecast: [
      { hour: 'Now', predicted: 45 },
      { hour: '+2h', predicted: 43 },
      { hour: '+4h', predicted: 41 },
      { hour: '+6h', predicted: 39 },
      { hour: '+8h', predicted: 37 },
      { hour: '+10h', predicted: 35 },
      { hour: '+12h', predicted: 33 },
    ]
  };

  const getDialsInfo = () => {
    let healthStatus = 'Suboptimal';
    let healthColor = 'text-amber-600';
    if (p.healthIndex >= 85) {
      healthStatus = 'Optimal Health';
      healthColor = 'text-emerald-600';
    } else if (p.healthIndex < 60) {
    ...
    if (p.diseaseRiskScore < 20) {
      riskStatus = 'Optimal Condition';
      riskColor = 'text-emerald-600';
    } else if (p.diseaseRiskScore < 45) {
      riskStatus = 'Low pathogen presence';
      riskColor = 'text-amber-600';
    }

    let waterStatus = 'Irrigation Standby';
    let waterColor = 'text-emerald-600';
    if (p.waterRequirement !== '0 Liters') {
      waterStatus = 'Deficit Detected';
      waterColor = 'text-amber-600';
    }

    return {
      health: { label: healthStatus, color: healthColor },
      risk: { label: riskStatus, color: riskColor },
      water: { label: waterStatus, color: waterColor },
    };
  };

  const statusInfo = getDialsInfo();
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card rounded-2xl p-6 border border-slate-200/80 text-center space-y-3">
          ...
          <span className={`text-xs font-bold ${statusInfo.health.color}`}>{statusInfo.health.label}</span>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 border border-slate-200/80 text-center space-y-3">
          ...
          <span className={`text-xs font-bold ${statusInfo.risk.color}`}>{statusInfo.risk.label}</span>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 border border-slate-200/80 text-center space-y-3">
          ...
          <span className={`text-xs font-bold ${statusInfo.water.color}`}>{statusInfo.water.label}</span>
        </div>
      </div>
